...
$thumbnail1 = "/assets/images/nuc12_right.webp";
$thumbnail2 = "/assets/images/nuc12_front.webp";
$thumbnail3 = "/assets/images/nuc12_back.webp";

// Create MINIPC list
$minipc_list = [$minipc_nuc12i3,$minipc_nuc12i5,$minipc_nuc12i7];

// Create RAM list
$ram_list = [$lexar_8g_3200_laptop,$lexar_16g_3200_laptop];

// Create SSD list
$ssd_list = [$lexar_256g_m2,$lexar_512g_m2];
...
$cart_list = [$minipc_nuc12i3, // MINIPC
              $lexar_8g_3200_laptop,    // RAM
              $lexar_256g_m2,  // SSD
...
<head>
    ...
    function updateCart(){
        // Get all MINIPC buttons
        const allMinipcButtons = document.querySelectorAll('input[name="minipc"]');
        // Lấy giá từ MINIPC được chọn
        const selectedMinipc = document.querySelector('input[name="minipc"]:checked');
        // Filter out the selected radio button to get the unselected ones
        const unselectedMinipcs = Array.from(allMinipcButtons).filter(button => button !== selectedMinipc);
        if (selectedMinipc) {
            const minipcOption = selectedMinipc.closest('.config-option');
            let minipcProduct = JSON.parse(minipcOption.dataset.product);
            let minipcPrice = minipcProduct.price;
            let optionPrice = minipcOption.querySelector('.option-price');
            // update cart
            cart.set(minipcProduct.type,minipcProduct);
            // clear price display
            optionPrice.innerHTML = "";

            // Iterate over each unselected radio button
            unselectedMinipcs.forEach(button => {
                // find the parent option
                let minipcOptionUnselected = button.closest('.config-option');
                // get unselected price
                let minipcProductUnselected = JSON.parse(minipcOptionUnselected.dataset.product);
                let minipcPriceUnselected = minipcProductUnselected.price;
                // get price difference
                let priceDiff = minipcPriceUnselected - minipcPrice;
                // update display price difference
                let optionPriceUnselected = minipcOptionUnselected.querySelector('.option-price');
    ...
    function updateDisplay(){
        ...
        if (cart.has('minipc')){
            document.getElementById('table_minipc').innerHTML = cart.get('minipc').name;
            document.getElementById('table_port').innerHTML = cart.get('minipc').note.port;
            document.getElementById('table_display').innerHTML = cart.get('minipc').note.display;
            document.getElementById('payment-minipc').innerHTML = "MINIPC: " + cart.get('minipc').name + "<br>";
        }
        else{
            document.getElementById('table_minipc').parentElement.style.display = 'none';
            ...
            document.getElementById('payment-minipc').style.display = "none";
        }
            ...
            <div class="product-main-content">
                <div class="row">
                    ...
                    <table>
                        ...
                        <tr>
                            <td class="spec-name">MiniPC</td>
                            <td class="spec-value" id="table_minipc"></td>
                        </tr>
                        <tr>
                ...
                .spec-name {
                    ...
                    <div class="product-column product-config">
                        <div class="product-header">
                            ...
                            <div>
                            <strong>Hiệu năng mạnh mẽ, thiết kế nhỏ gọn, tối ưu mọi nhu cầu!</strong>
                            <br>
                            ROSA NUC12 PC là sự kết hợp hoàn hảo giữa hiệu suất và tính linh hoạt, với các tùy chọn CPU từ Intel Core i3-1220P cho các tác vụ văn phòng cơ bản, Intel Core i5-1240P đáp ứng đa nhiệm hiệu quả, đến Intel Core i7-1260P dành cho những nhu cầu xử lý phức tạp và hiệu năng cao. Máy tính này nổi bật với thiết kế nhỏ gọn, tiết kiệm không gian, cùng khả năng kết nối hiện đại, hỗ trợ tối ưu hóa công việc văn phòng và sử dụng đa mục đích. Đây chính là sự lựa chọn thông minh cho mọi doanh nghiệp và cá nhân muốn tối đa hóa hiệu quả làm việc trong một không gian gọn gàng, hiện đại.
                            <br><br>
                            </div>
                        </div>
                        <h3>Hệ Điều Hành</h3>
                        ...
                        <div class="config-sections">
                            ...
                            <div class="config-section">
                                ...
                                <div class="config-options">
                                    <?php
                                    foreach ($minipc_list as $index => $product) {
                                        if ($index === 0){
                                ...
                                <tr>
                                    <td class="product-details">
                                        ...
                                        <div>
                                            ...
                                            <span class="prodcut-name" id="payment-minipc"></span>
...
<script>
            ...
            <div class="modal fade" id="cpuModal" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">CPU phù hợp với bạn</h5>
                            <button type="button" class="close" data-dismiss="modal">x</button>
                        </div>
                        <div class="modal-body">
                        <p class="mb-4">CPU giống như bộ não của máy tính, điều phối tất cả các thành phần để PC
                                hoạt động.</p>
                        <div class="component-options">
                                <div class="option-item">
                                    <h4>I3-1220P:</h4>
                                    <ul>
                                        <li>Phù hợp cho các tác vụ văn phòng cơ bản như xử lý văn bản, bảng tính, và duyệt web. Với 10 nhân (2 nhân P, 8 nhân E) và hiệu suất ổn định, đây là lựa chọn lý tưởng cho công việc hàng ngày mà vẫn tiết kiệm năng lượng.</li>
                                    </ul>
                                </div>
                                ...
                                <div class="option-item">
                                    <h4>I5-1240P</h4>
                                    <ul>
                                        <li>Tăng cường hiệu năng với 12 nhân (4 nhân P, 8 nhân E), CPU này đáp ứng mượt mà các nhu cầu đa nhiệm, làm việc trên nhiều ứng dụng cùng lúc hoặc xử lý các tác vụ nặng hơn như chỉnh sửa hình ảnh cơ bản và quản lý dữ liệu lớn.</li>
                                    </ul>
                                </div>
                                ...
                                <div class="option-item">
                                    <h4>I7-1260P</h4>
                                    <ul>
                                        <li>Với sức mạnh vượt trội từ 12 nhân (4 nhân P, 8 nhân E) và xung nhịp cao hơn, đây là lựa chọn hoàn hảo cho người dùng chuyên nghiệp, hỗ trợ xử lý đồ họa, chỉnh sửa video, và các tác vụ phức tạp một cách nhanh chóng và hiệu quả.</li>
                                    </ul>
                                </div>
    ...
    <div class="specs-container">
        ...
        <h2>Tổng Quan Sản Phẩm</h2>
            <p style="color:black;">
                ROSA NUC12 PC mang đến sự kết hợp hoàn hảo giữa hiệu suất mạnh mẽ và tính linh hoạt vượt trội, đáp ứng mọi nhu cầu sử dụng từ cơ bản đến chuyên sâu. Với các tùy chọn CPU đa dạng, từ Intel Core i3-1220P lý tưởng cho các công việc văn phòng thường ngày, Intel Core i5-1240P hỗ trợ đa nhiệm mượt mà, đến Intel Core i7-1260P dành cho các tác vụ đòi hỏi xử lý phức tạp và hiệu năng cao, máy tính này đảm bảo phù hợp với mọi đối tượng người dùng.
                <br>
                Nổi bật với thiết kế nhỏ gọn và tiết kiệm không gian, ROSA NUC12 PC không chỉ dễ dàng tích hợp vào bất kỳ môi trường làm việc nào mà còn được trang bị khả năng kết nối hiện đại, đáp ứng nhu cầu sử dụng đa mục đích. Đây chính là giải pháp tối ưu cho các doanh nghiệp và cá nhân muốn nâng cao hiệu quả công việc trong một không gian làm việc hiện đại và gọn gàng.
            </p>
        </div>
    </div>
